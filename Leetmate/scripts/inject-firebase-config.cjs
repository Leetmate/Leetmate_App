...
const TEMPLATE_PATH = path.join(ROOT, 'scripts', 'firebase-config.template.js');
const OUT_PATH = path.join(ROOT, 'scripts', 'firebase-config.js');
...
if (!fs.existsSync(TEMPLATE_PATH)) {
  console.error('Missing scripts/firebase-config.template.js');
  process.exit(1);
}
...
console.log('Wrote scripts/firebase-config.js from .env variables.');
...
console.log('');
console.log('Committed source uses variable names in scripts/firebase-config.template.js;');
console.log('scripts/firebase-config.js is generated and contains values from .env.');
